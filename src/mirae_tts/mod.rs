...
impl MiraeTTS {
    pub fn new(setting: &Setting) -> Result<Self> {
        ...
        let client = reqwest::ClientBuilder::new()
            ...
            .user_agent("discord-tts-miraetts/0.0.0")
            .build()
            .unwrap();
...
impl TtsService for MiraeTTS {
    async fn tts(&self, style_id: &str, text: &str) -> Result<Vec<u8>> {
        ...
        let resp = self
            ...
            .send()
            .await
            .context("Failed to post /api/synthesize (connect)")?
            .error_for_status()
            .context("Failed to post /api/synthesize (status_code)")?;

        let resp = resp
            .bytes()
            .await
            .context("Failed to post /api/synthesize (body)")?;
